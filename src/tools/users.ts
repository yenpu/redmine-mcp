...
export function registerUserTools(server: McpServer, client: RedmineClient): void {
  ...
  server.tool(
    ...
    {
      login: z.string().max(60).describe('Login name (required)'),
      firstname: z.string().describe('First name (required)'),
      lastname: z.string().describe('Last name (required)'),
      mail: z.string().email().max(254).describe('Email address (required)'),
      password: z.string().min(8).max(128).optional().describe('Password (if not set, user must use password reset)'),
  ...
  server.tool(
    ...
    {
      id: z.number().int().describe('User ID'),
      login: z.string().max(60).optional().describe('Login name'),
      firstname: z.string().optional().describe('First name'),
      lastname: z.string().optional().describe('Last name'),
      mail: z.string().email().max(254).optional().describe('Email address'),
      password: z.string().min(8).max(128).optional().describe('New password'),
      auth_source_id: z.number().int().optional().describe('Authentication source ID'),
      mail_notification: z.string().optional().describe('Email notification preference'),
